...
use \Nne\Core\Module\Manager as ModuleManager;
use \Nne\Core\Menu\MenuManager ;
use \Nne\Core\Facade\Facade;
...
/**
 * Nne Bootstrapper, initialize all the thing needed on the start
 */
class Bootstrap
{
    protected $app;
    ...
    public function bootSentry($config){
        ...
        $this->app->container->singleton('sentry', function() use ($app, $config,$hasherProvider,$userProvider,$groupProvider,$throttleProvider){
            return new Sentry(
                $userProvider,
                $groupProvider,
                $throttleProvider,
